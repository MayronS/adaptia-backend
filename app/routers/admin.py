...
async def editar_topico(
    ...
    # Se o pré-requisito mudou, atualiza os progressos dos alunos
    if prereq_antigo != prereq_novo:
        ...
        if prereq_novo is None:
            # Pré-req removido: desbloqueia todos que estavam bloqueados
            for p in progressos:
                if p.status == StatusProgresso.bloqueado:
                    p.status = StatusProgresso.disponivel
        else:
            # Pré-req adicionado: bloqueia alunos que não concluíram o pré-req
